...
ACE_RCSID (DLL_ORB,
           Test_Server_Module,
           "Test_Server_Module.cpp,v 1.3 2001/08/13 01:01:54 othman Exp")
...
Test_Server_Module::init (int argc, ACE_TCHAR *argv[])
{
  ...
  ACE_TRY
    {
      ...
      // Advertise this object in the IOR table.
      ...
      iorTable->bind ("Test_Server", ior.in ());
  ...
  if(ACE_OS::hostname (hostName,MAXHOSTNAMELEN+1) != 0) {
     ACE_ERROR_RETURN ((LM_ERROR,
                         "%p\n",
                         "ERROR: Unable to get local hostname "),
                        -1);
  }
  char buffer[256];
  ...
  //ACE_OS::sprintf(buffer, "file://test.ior"); // this will work
  ACE_OS::sprintf(buffer, "corbaloc:iiop:%s:6464/Test_Server", hostName); //this will deadlock on narrow
  
  CORBA::Object_var obj = this->orb_->string_to_object (buffer); 
  ACE_DEBUG((LM_DEBUG, "Connecting with: %s\n", buffer));

  ACE_DEBUG((LM_DEBUG, "Attempting to narrow...\n"));
  Test_var test = Test::_narrow (obj.in ());
  
  if (CORBA::is_nil (test.in ()))
  {
    ACE_ERROR_RETURN ((LM_DEBUG,
      ACE_TEXT ("Nil Test reference\n")),
      1);
  }

  ACE_DEBUG((LM_DEBUG, "Successfully made a corba call\n"));
