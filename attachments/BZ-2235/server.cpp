// server.cpp,v 1.18 2005/06/14 14:29:15 jwillemsen Exp
...
ACE_RCSID(Thread_Pool, server, "server.cpp,v 1.18 2005/06/14 14:29:15 jwillemsen Exp")
...
create_POA_and_register_servant (CORBA::Policy_ptr threadpool_policy,
                                 ...
                                 PortableServer::POA_ptr &poa_out,
                                 test_i * &servant
                                 ACE_ENV_ARG_DECL)
{
  ...
  // "send out the POA"
  poa_out = PortableServer::POA::_duplicate(poa);
...
//       this->orb_->run (ACE_ENV_SINGLE_ARG_PARAMETER);
      ...
      PortableServer::ObjectId_var oid = first_poa->servant_to_id(first_servant);
      ACE_DEBUG((LM_DEBUG, ACE_LIB_TEXT ("file %N, line %l. deactivate the first servant \n")));
      first_poa->deactivate_object(oid.in());
      // deactivate the POA Manager
      ACE_DEBUG((LM_DEBUG, ACE_LIB_TEXT ("file %N, line %l. deactivate the POA Manager \n")));
      poa_manager->deactivate(true, true);  // deactivate, wait for completion
      // destroy the POA
      // shutdown the first threadpool
      ACE_DEBUG((LM_DEBUG, ACE_LIB_TEXT ("file %N, line %l. shutdown threadpool 1 \n")));
      rt_orb->destroy_threadpool(threadpool_id_1);
      
      ACE_DEBUG((LM_DEBUG, ACE_LIB_TEXT ("file %N, line %l. shutdown threadpool 2 \n")));
      rt_orb->destroy_threadpool(threadpool_id_2);
