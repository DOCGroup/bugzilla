...
#include "ECG_ConsumerEventChannelControl.h"
#include "ECG_Reactive_ConsumerEventChannelControl.h"
#include "ECG_Reconnect_ConsumerEventChannelControl.h"
...
TAO_ECG_ConsumerEventChannelControl*
TAO_EC_Gateway_IIOP_Factory::create_consumer_eventchannel_control (TAO_EC_Gateway_IIOP* gateway)
{
  if (this->consumer_ec_control_ == 0)
    return new TAO_ECG_ConsumerEventChannelControl ();
  else if (this->consumer_ec_control_ == 1)
    {
      ...
      return new TAO_ECG_Reactive_ConsumerEventChannelControl (rate, consumer_ec_control_timeout_, gateway, orb.in ());
    }
  else if (this->consumer_ec_control_ == 2)
    {
      ...
      CORBA::ORB_var orb =
        CORBA::ORB_init (argc, argv, this->orbid_.c_str ());
      ACE_Time_Value rate (0, this->consumer_ec_control_period_);
      return new TAO_ECG_Reconnect_ConsumerEventChannelControl (rate, consumer_ec_control_timeout_, gateway, orb.in ());
    }
  return 0;
}

void
TAO_EC_Gateway_IIOP_Factory::destroy_consumer_eventchannel_control (
  TAO_ECG_ConsumerEventChannelControl* x)
{
   delete x;
}
