 ...
 *
 * ACE_TMAIN          Proactor dispatcher
 * thread             thread
 * ----------------   --------------------------------
 *
 * init_ssl
 * connect
 *                    SH(1)::open   or SH(2)::open
 *                    SH(1)::read      SH(2)::read
 *                    SH(1)::write
 *                                     SH(2)::handle_read_stream
 *                                     SH(2)::write
 *                                     SH(2)::read
 *                    SH(1)::handle_read_stream
 *                    SH(1)::read
 * SH(1)::write or read (both cause do_SSL_read to fail)
 *
 ...
 * Proactor
 */
class Proactor : public ACE_Proactor {};
typedef ACE_Singleton<Proactor, ACE_SYNCH_RECURSIVE_MUTEX> 
    Proactor_Singleton;
#define PROACTOR Proactor_Singleton::instance()

class Proactor_Task : public ACE_Task_Base
{
public:
  virtual int svc (void);
};

typedef ACE_Singleton<Proactor_Task, ACE_SYNCH_RECURSIVE_MUTEX>
    Proactor_Task_Singleton;
#define PROACTOR_TASK Proactor_Task_Singleton::instance()

int
Proactor_Task::svc (void)
{
  PROACTOR->proactor_reset_event_loop ();
...
ACE_TMAIN (int, ACE_TCHAR *[])
{
  ...
  if (0 == ret)
  {
    ret = CONNECTOR->open (0, PROACTOR, 1);
