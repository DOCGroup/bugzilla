...
namespace {
  ...
  struct Result_Set {
    ...
    int analyze_reports (void)
    {
      ...
      ACE_DEBUG ((LM_DEBUG,
                  ACE_TEXT ("Total chunks (%d entries):\n"),
                  total_chunks));
...
ACE_TSS<npl::PreciseTimer> g_Timer;

ACE_Thread_Mutex g_lock_;
static ACE_Time_Value total_reactor_time;
static ACE_Time_Value total_read_time;
static ACE_Time_Value total_resume_handler_time;
...
{
  ACE_Time_Value reactor_tmp = g_Timer->elapsed();
  g_Timer->restart();
  char buf[BUFSIZ];
  ssize_t result = this->peer ().recv (buf, 26/*ACE_OS::strlen(ACE_ALPHABET)*/);
  ACE_Time_Value read_tmp = g_Timer->elapsed();
  g_Timer->restart();
  if (invoke_times_of_reactor == 1 && opt_resume) {
    int result = this->reactor()->resume_handler(h);  // This is for Dev_Poll_Reactor
	ACE_Time_Value tmp = g_Timer->elapsed();
	g_lock_.acquire();
	if (handle_count) {
	    total_reactor_time += reactor_tmp;
		total_read_time += read_tmp;
		total_resume_handler_time += tmp;
	}
       else{
	...
	g_lock_.release();
	if (result != 0) {
		std::cout << "Error at resume_handler" << std::endl;
	}
    //std::cout << g_Timer->elapsed() << std::endl;
  }
  if (result > 0)
    {
      if (opt_debug)
  ...
  for (int i = 0; i < opt_burden; i++) buf[0] = i % 0xFF;
    ACE_DEBUG((LM_DEBUG,
  ...
  //for (int j = 0; j < 10; j++) {
  //  g_Timer->restart();
  //  result = this->peer ().recv (buf, 26);
  //  read_tmp = g_Timer->elapsed();
...
sender (void *arg)
{
  ...
  std::cout << "Socket building" << std::endl;
      ...
      if (c.connect (socks[i], *connection_addr) == -1)
        {
          if (errno != ECONNREFUSED || i == 0)
            {
              std::cout << "Error socket build in loop" << std::endl;
              ACE_ERROR ((LM_ERROR,
                          ACE_TEXT ("(%t) conn %d %p\n"),
                          ACE_TEXT ("connect")));
              while (--i >= 0)
  ...
  if (i < opt_nconnections){
    std::cout << "Error socket build" << std::endl;
    return 0;
  }
  std::cout << "Done socket build" << std::endl;
  ...
  while (!tm->testcancel (me) && !fail)
    {
      ...
      for (i = opt_nconnections - 1; i >= 0; i--)
        {
...
run_main (int argc, ACE_TCHAR *argv[])
{
  ...
  ACE_Get_Opt getopt (argc, argv, ACE_TEXT ("c:s:t:b:dr"), 1);
  ...
  //FUZZ: enble check_for_lack_ACE_OS
    switch (c)
      ...
      case 'd':
        ...
        break;
      case 'b':
        opt_burden = ACE_OS::atoi (getopt.opt_arg ());
        break;
      case 'r':
        opt_resume = 1;
        break;
...
invoke_times_of_reactor++;
  ...
  if (handle_count == 0) ++handle_count;
  ACE_UINT64 tv;
  (total_reactor_time + total_read_time + total_resume_handler_time).to_usec(tv);
  std::cout << "Elapsed total = " <<
	total_reactor_time + total_read_time + total_resume_handler_time << " handle_times = " <<
	handle_count  << " read_times" << read_count << ", AVR= " << tv / handle_count << ", Connection=" << opt_nconnections << std::endl;
  std::cout << "Reactor = " <<
	total_reactor_time << ", Read = " << total_read_time << ", Resume = " << total_resume_handler_time << std::endl;
