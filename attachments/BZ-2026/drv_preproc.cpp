// drv_preproc.cpp,v 1.126 2004/08/26 17:38:20 parsons Exp
...
DRV_pre_proc (const char *myfile)
{
  ...
  //MLM - the file descriptor tf_fd is never used and is left open
  // this causes a problem later with the ACE_OS::open call. The unlink (line 770) later works
  // but not until after all file handles are closed, so the create exclusive fails.
  ...
  //MLM - since I do not know what someones intent was I will simply close it.  
  //  The use if this file descriptor should just be removed though.
  ACE_OS::close (tf_fd);
  ...
  if (output_arg_format == 0)
    {
      ...
      if (fd == ACE_INVALID_HANDLE)
        {
          ACE_ERROR ((LM_ERROR,
                      "%s: xx cannot open temp file \"%s\" for writing\n",
                      idl_global->prog_name (),
                      t_file));
