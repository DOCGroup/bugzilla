// $Id: unit_test.cpp,v 1.3 2009/02/04 14:00:23 rmora Exp $
...
ACE_RCSID(Bug_2494_Regression, server, "$Id: unit_test.cpp,v 1.3 2009/02/04 14:00:23 rmora Exp $")
...
#define TEST_ASSERT(X)                        \
  do { if(!(X)) { \
      ACE_ERROR ((LM_ERROR, ACE_LIB_TEXT ("TEST_ASSERT: (%P|%t) file %N, line %l assertion failed for '%s'.%a\n"), \
...
////////////////////////////////////////////////////////////////////////
//   Class Worker inline implementation
//   on AIX gives error if we declare 
//   ACE_TSS<Worker> workers;
//   before defining memory structure of Class Worker, 
//   Modified to avoid an errors in Synch_T.cpp", line 274.3 while 
//   compling on AIX.5.1_RISC_vacpp50 platform. 
////////////////////////////////////////////////////////////////////////
int Worker::svc (void)
{
  if (debug)
...
int Worker::process_cmd (void)
{
  ...
  Command* cmd = ACE_dynamic_cast (Command*, mb);
  ACE_ASSERT (cmd != 0);
  cmd->execute (this);
  cmd->release ();
  return 0;
...
public:
  ...
  virtual int send_message (TAO_OutputCDR &,
                            TAO_Stub * = 0,
                            int  = TAO_Transport::TAO_TWOWAY_REQUEST,
                            ACE_Time_Value * = 0)
  {
    return 0;
  }
...
// class Complete_Event: public Command
// {
// public:
//   Complete_Event(Test_LF_Event& event)
//     : event_ (event)
//   {}

//   virtual int execute (Worker*)
//   {
//     ACE_DEBUG ((LM_DEBUG, "(%P|%t) Executing Complete_Event cmd\n"));
